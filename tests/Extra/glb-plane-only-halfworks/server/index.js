...
class GLBParser {
  ...
  async parseGLB(buffer) {
    ...
    // Find nodes with "Body" or "Floor" in name for colliders
    root.listNodes().forEach(node => {
      const nodeName = node.getName();
      if (nodeName && (nodeName.includes('Body') || nodeName === 'Floor')) {
...
wss.on('connection', (ws) => {
  ...
  const spawnPosition = { x: -200, y: 5, z: -200 }; // Match client spawn position, extremely far from plane at (10,3,0)
  ...
  players.set(playerId, { ws, position: spawnPosition, onPlane: false });
